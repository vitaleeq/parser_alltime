...
#cookie_dynamic = requests.request("GET", url).headers['Set-Cookie']
#cookies = 'srv_id=srv4'

def get_product_id(vendor_code):
    url = 'https://www.alltime.ru/search/?NAME=' + vendor_code
    page = requests.get(url)
    #print(page.headers['Set-Cookie'])
    #global cookie_dynamic
    #cookie_dynamic = page.headers['Set-Cookie']
    ...
    try:
        ...
        '''
            if response.text == '':
                ...
                if 'srv_id=srv4' in headers['Cookie']:
                    headers['Cookie'] = headers['Cookie'].replace('srv_id=srv4', 'srv_id=srv7')
                elif 'srv_id=srv7' in headers['Cookie']:
                    headers['Cookie'] = headers['Cookie'].replace('srv_id=srv7', 'srv_id=srv4')
                else:
                    headers['Cookie'] += 'srv_id=srv4'
            else:
                print(f'{response.text=}')
                break
            '''
